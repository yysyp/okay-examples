...
import ps.demo.commupload.batchjob.tasklets.DeleteInputCsvTasklet;
...
//  private Resource inputFeed;
  ...
  @Bean
  public Step step2(PlatformTransactionManager txManager, DeleteInputCsvTasklet deleteInputCsvTasklet) {

    var name = "DELETE CSV FILE";
    var builder = new StepBuilder(name, jobRepository);
    return builder
        .tasklet(deleteInputCsvTasklet, txManager)
        .build();
  }

  @Bean
  @StepScope
  public DeleteInputCsvTasklet deleteInputCsvTasklet(@Value("#{jobParameters['tempFile']}") String tempFilePath) {
    DeleteInputCsvTasklet tasklet = new DeleteInputCsvTasklet();
    tasklet.setResources(new Resource[] {new FileSystemResource(tempFilePath)});
    return tasklet;
  }

  @Bean
  @StepScope
  public FlatFileItemReader<Person> csvFileReader(@Value("#{jobParameters['tempFile']}") String tempFilePath,
      LineMapper<Person> lineMapper) {
    var itemReader = new FlatFileItemReader<Person>();
    itemReader.setLineMapper(lineMapper);
    itemReader.setResource(new FileSystemResource(tempFilePath));
    return itemReader;
  }
